...
public interface EmpAppCheckListDetlRepository extends JpaRepository<EmpAppCheckListDetl, Integer> {
	...
	@Query("SELECT eacld FROM EmpAppCheckListDetl eacld WHERE eacld.isActive = :isActive")
	List<EmpAppCheckListDetl> findByIsActive(@Param("isActive") Integer isActive);
	/**
